...
namespace MusikPlayer.Design
{
    ...
    public class iScrollControl : Panel
    {
        ...
        private class VScrollBarEx : VScrollBar
        {
            public event EventHandler UserInteraction;

            protected override void WndProc(ref Message m)
            {
                if (m.Msg == 0x201) // catch mouse down
                    if (UserInteraction != null)
                        UserInteraction(this, EventArgs.Empty);

                // No matter what, we must always pass on the windows message
                base.WndProc(ref m);
            }
        }

        private class HScrollBarEx : HScrollBar
        {
            public event EventHandler UserInteraction;
        ...
        protected virtual void CreateDockControls()
        {
            ...
            mHScrollBar.UserInteraction += new EventHandler(ScrollBar_UserInteraction);
            mVScrollBar.UserInteraction += new EventHandler(ScrollBar_UserInteraction);
        ...
        /// <summary>
        /// Override this method to get notified whenever the user touches the control
        /// </summary>
        ...
        /// <summary>
        /// This method is called whenever the user messes with the scrollbars
        /// </summary>
        private void ScrollBar_UserInteraction(object sender, EventArgs e)
        {
            OnUserInteraction();
        }

        /// <summary>
        /// We also want to notify the derived class whenever the user clicks in the grid
        /// </summary>
        /// <param name="e"></param>
        ...
        private HScrollBarEx mHScrollBar = new HScrollBarEx();
        private VScrollBarEx mVScrollBar = new VScrollBarEx();
        /// <summary>
        ...
        public virtual Point CustomScrollPosition
        {
            ...
            set
            {
                if (HScrollBarVisible)
                {
                    if (value.X > mHScrollBar.Maximum)
                        mHScrollBar.Value = mHScrollBar.Maximum;
                    else if (value.X > 0)
                ...
                if (VScrollBarVisible)
                {
                    if (value.Y > mVScrollBar.Maximum)
        ...
        /// </summary>
        /// //Jay 10/24/3008: Made protected so it can be accessed by GridViewer
        protected void RecalcCustomScrollBars()
        {
            SuspendLayout();
        ...
        int GetScrollRows(int displayHeight)
        {
            return displayHeight;
        }

        int GetScrollColumns(int displayWidth)
        {
            return displayWidth;
        }
        ...
        public virtual void CustomScrollWheel(int rotationDelta)
        {
            if (rotationDelta >= 120 || rotationDelta <= -120)
            {
                if (VScrollBarVisible)
                {
                    ...
                    {
                        // We are not showing the last row now
                        OnUserInteraction();
                    }
